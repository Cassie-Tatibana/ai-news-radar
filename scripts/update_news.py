...
OFFICIAL_AI_FEEDS: tuple[dict[str, str], ...] = (
    {
        "title": "OpenAI Blog",
        "xml_url": "https://openai.com/blog/rss.xml",
        "html_url": "https://openai.com/blog",
    },
    {
...
def parse_cursor_changelog_items(markdown_text: str, now: datetime) -> list[RawItem]:
    site_id = "cursor"
    ...
    title_pattern = re.compile(r"^\s*# \[(.*?)\]\((https://cursor\.com/changelog/[^)]+)\)\s*$", re.M)
    date_matches = list(date_pattern.finditer(markdown_text or ""))
    title_matches = list(title_pattern.finditer(markdown_text or ""))

    for idx, date_match in enumerate(date_matches):
        label = date_match.group(1)
        date_url = date_match.group(2)
        next_date_start = date_matches[idx + 1].start() if idx + 1 < len(date_matches) else len(markdown_text or "")
        title_match = next(
            (
                match
                for match in title_matches
                if date_match.end() <= match.start() < next_date_start
            ),
            None,
        )
        ...
        title_url = title_match.group(2)
        url = (title_url or date_url).strip()
        if not url or url in seen:
            continue
        seen.add(url)
        parsed_label = re.search(r"([A-Z][a-z]{2}\s+\d{1,2},\s+\d{4})", label)
        published = parse_date_any(parsed_label.group(1) if parsed_label else label, now)
        if not published:
            continue
        if now and published < now - timedelta(days=OFFICIAL_AI_MAX_AGE_DAYS):
            continue
        ...
        out.append(
            RawItem(
                ...
                url=url,
                published_at=published,
                meta={"feed_home": "https://cursor.com/changelog"},
            )
        )
...
def fetch_cursor_changelog(session: requests.Session, now: datetime) -> list[RawItem]:
    ...
    resp.raise_for_status()
    out = parse_cursor_changelog_items(resp.text, now)
    if not out:
...
def main() -> int:
    ...
    parser.add_argument("--archive-days", type=int, default=30, help="Keep archive for N days")
