...
这是一个自动更新的 24 小时 AI 资讯雷达，默认部署到 GitHub Pages。

基于原作者项目二次定制：

- 原项目仓库：`https://github.com/LearnPrompt/ai-news-radar`
- 原在线页面：`https://learnprompt.github.io/ai-news-radar/`
- 当前版本默认公开源流程不需要任何付费 API，也不需要你自己维护服务器

普通用户直接打开网页，看最近 24 小时 AI 编程工具、Agents、MCP 与行业趋势更新。维护者可以 fork 这个仓库，接入自己的 OPML/RSS、公开 feed、静态页面或 AgentMail 邮箱情报。Codex / Claude Code 这类 Agent 可以使用项目内置的 **伯乐Skill**，继续帮你判断信息源、维护抓取逻辑、部署 GitHub Pages。
...
- 费用说明：默认配置免费；只有你主动开启 X API、AgentMail 或其他私有高级源时，才可能产生额外费用
...
```bash
git clone https://github.com/Cassie-Tatibana/ai-news-radar.git
cd ai-news-radar
python3 -m venv .venv
